...
export default class Rook extends Chessman {
    ...
    constructor(index: number, type: PlayerType = 'black') {
        super(index, type, 'rook');

        this.type = type;
        this._sprite = Sprite.from(
            `chessman/${Rook.name.toLowerCase()}_${type === 'black' ? 'b' : 'w'}${Singleton.SPRITES_TYPE}`,
        );
    ...
    onClick = () => {
        super.onClick(this.rookPossibleMoves);
    };
